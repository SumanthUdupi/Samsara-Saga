  ...
  try {
    ...
    switch (actionType) {
      case 'LOOK_AROUND': {
        ...
        const prompt = `You are a master storyteller crafting immersive narratives for a text-based game inspired by Hindu philosophy. The player is a soul on a journey to moksha, their perception heightened to the subtle currents of the cosmos.
...
Your Observation:`;
      ...
      case 'MEDITATE': {
        ...
        const prompt = `You are a wise Guru guiding a soul's meditation in a text-based game about Hindu philosophy. The soul is reflecting inward to better understand its nature and its place in the cosmos.

**Player Context**:
- **Nakshatra**: ${nakshatra?.name || 'Unknown'}
- **Core Nature**: ${nakshatra?.nature || 'Unknown'} (e.g., Cruel, Gentle, Fixed, Movable)
- **Current Karma**: ${newKarmaScore}
